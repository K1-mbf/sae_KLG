...
public class HandlerConsumers implements Handler<RoutingContext> {
    ...
    private JsonObject getJsonById(String id){
        Object[] sql = (Object[]) db.createNativeQuery("SELECT s.id,s.name,s.description,s.dtype,s.grid,c.max_power FROM Sensor as s, consumer as c WHERE s.id = " +id + " AND s.id = c.id").getSingleResult();
